...
int main(int argc, char **argv) {
    ...
    double yaw_rate = 0.785398; //rad/sec (45 Degree/sec) yaw rate command
    double time_1_sec = 1.0; // should move 1  meters or  45 Degrees in 1 seconds
	double time_2_sec = 2.0; // should move 3  meters or  90 Degrees in 2 seconds
    double time_3_sec = 3.0; // should move 3  meters or 135 Degrees in 3 seconds
 ...
 //         }  
    ...
    twist_cmd.angular.z=0.0; //Should be facing east now, stop spinning in place 
    twist_cmd.linear.x=speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<4.5) {
          twist_commander.publish(twist_cmd);
    ...
    twist_cmd.angular.z=0.0; //Should be facing north now, stop spinning in place 
    twist_cmd.linear.x=speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<2.3) {
          twist_commander.publish(twist_cmd);
 ...
 twist_cmd.linear.x=0.0; //stop moving forward
    ...
    while(timer<time_2_sec) {
          ...
          twist_cmd.angular.z=0.0; //Should be facing west now, stop spinning in place 
    twist_cmd.linear.x=speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<5.5) {
          twist_commander.publish(twist_cmd);
...
twist_cmd.angular.z=0.0; //Should be facing northwest now, stop spinning in place 
    twist_cmd.linear.x=speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<3.2) {
          twist_commander.publish(twist_cmd);
